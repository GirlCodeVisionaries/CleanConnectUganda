...
function getPrunedPnpmWorkspaceSettings(workspaceRootPath, prunedLockfileContent, precomputed) {
    ...
    try {
        ...
        logger_1.logger.warn('Could not read the workspace root pnpm-workspace.yaml; the pruned output will not declare pnpm install settings (build-script approvals, supportedArchitectures, patchedDependencies).');
        return null;
    }
    const settings = {};
...
function getPnpmMajorOrWarn(workspaceRootPath) {
    ...
    if (pnpmMajor === null) {
        logger_1.logger.warn('Could not determine the pnpm version. The pruned output will not carry pnpm build-script approvals, supportedArchitectures, or patchedDependencies declarations; patch files still ship.');
    }
    return pnpmMajor;
}
/**
